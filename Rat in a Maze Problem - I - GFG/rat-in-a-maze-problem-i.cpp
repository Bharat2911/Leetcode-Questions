   ...
   void solve(int i,int j,vector<vector<int>>&arr,int n,string move,vector<string>&ans,vector<vector<int>>&vis)
    {
        ...
        ///itrate in four directions
        //downward
        if(i+1<n  && !vis[i+1][j] && arr[i+1][j]==1)
        {
            vis[i][j]=1;
            solve(i+1,j,arr,n,move+'D',ans,vis);
            vis[i][j]=0;
        }
        ...
        {
            vis[i][j]=1;
            solve(i,j-1,arr,n,move+'L',ans,vis);
            vis[i][j]=0;
        }
        ...
        {
            vis[i][j]=1;
            solve(i,j+1,arr,n,move+'R',ans,vis);
            vis[i][j]=0;//for backtraking
        }
        ...
        {
            vis[i][j]=1;
            solve(i-1,j,arr,n,move+'U',ans,vis);
            vis[i][j]=0;
        }
    }
